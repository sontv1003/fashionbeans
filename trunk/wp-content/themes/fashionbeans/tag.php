...
get_header(); ?>
<div class="breadcrumb"><a href="<?php echo esc_url( home_url( '/' ) ); ?>">Trang chủ</a> > Thẻ > <?php printf( __( 'Kết quả tìm kiếm:') ); ?></div>
<h1 class="searchCategory"><?php printf( __( 'Kết quả cho thẻ: %s', 'twentyeleven' ), '<span>' . single_tag_title( '', false ) . '</span>' );?></h1>
                <div id="results">
                    <div class="gsc-above-wrapper-area"></div>
			<?php
                        query_posts(array('tag_id' => $tag_id,'post_status' => 'publish','posts_per_page'  => 10,'paged' => $paged));
                        if ( have_posts() ) : ?>
				<?php /* Start the Loop */ ?>
